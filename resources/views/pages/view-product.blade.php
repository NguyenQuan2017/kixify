            ...
            <div class="checkboxes-wrapper">
               <div class="form-checkboxes">
                  ...
                  @foreach($cate as $cat)
                     ...
                     <div class="option" for="category-0"><input type="checkbox" name="category[]" id="category-0" value="Adidas"   class="form-checkbox" /> <span class="unbxd-checkbox-label">{{$item['name_cate']}}</span></div>
                   @endforeach  
                  @endforeach 
                  </div>
               </div>
            ...
            <div class="sidebar-contained col-xs-12 col-sm-5 col-md-5 clear-padding-right add-padding-right">
               <div class="sidebar-contained-inner">
                  <div id='release-event-date-style-wraper'>
                     <h1 class="release-event-title">{{$products['name_product']}}</h1>
                     <div class="release-event-style">{{$products['release_style']}}</div>
                  </div>
                  <div id='release-event-release-date-wraper'>
                     <div class="release-event-date-label">Release Date</div>
                     <div class="release-event-release-date">{{Carbon\Carbon::parse($products['release_date'])->toFormattedDateString()}} </div>
                  </div>
                  <div id='release-event-style-code-wraper'>
                     <div class="release-event-date-label">Style Code</div>
                     <div class="release-event-release-date">{{$products['style_code']}}</div>
                  </div>
      ...
      <div>
         <div class="view view-category-listing view-id-category_listing">
            <div class="view-content-container">
               ...
               <input type="hidden" name="cat_product_id" value="{{$products->id}}">
                  ...
                  <div class="col-xs-6 col-sm-3 col-md-3 product-teaser" unbxdattr="product" unbxdparam_prank="1" unbxdparam_sku="8bea56678e877e6a85d13b79e9dc731b">
                     <div class="product-info">
                        <div class="product-titles hidden-xs">
                           <div class="product-title"><a href="{{route('product',[$product_cats->id,$product_cats->slug])}}">{{strtolower($product_cats['name_product'])}}</a></div>
                        </div>
                     </div>
                     <a href="{{route('product',[$product_cats->id,$product_cats->slug])}}" class="thumbnail"><span class="product-price">${{$product_cats['price']}}</span><img src="{{url('public/images/products/'.$product_cats->image)}}" alt="Adidas nmd r1 zebra black prim..." title="" width="180" height="190" class="img-responsive imagecache imagecache-product_list"></a>
                     <!--<div > </div>-->
                  </div>
                @endforeach  
                  <div class="clearfix"></div>
